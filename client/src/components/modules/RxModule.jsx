...
const RxModule = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                console.error(err);
                setError('Failed to load Rx staff');
                setLoading(false);
            }
        };
    ...
    if (loading) return <div className="p-6 text-slate-600 dark:text-slate-400">Loading Rx Staff...</div>;
    ...
    return (
        <div className="p-6 bg-white dark:bg-slate-800 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700">
            <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-6">DOH Rx Team</h2>

            {users.length === 0 ? (
                <p className="text-slate-500">No active staff found in DOH Rx branch.</p>
            ) : (
                    ...
                    {users.map(user => (
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-xs text-emerald-500 font-semibold uppercase">{user.post}</p>
